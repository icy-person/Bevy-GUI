...
impl Plugin for BevyGuiPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(EguiPlugin::default())
            ...
            .add_message::<HistoryCommandEvent>()
...
fn load_initial_editor_scene(
    ...
    let Some(relative) = project.main_scene.as_ref() else {
        return;
    };
    let path = project.root.join(relative);
    let Ok(document) = crate::scene::load_scene(&path) else {
        return;
    };
    let spawned = crate::scene::spawn_scene_with_renderables(
        &mut commands,
        &mut meshes,
        &mut materials,
        &asset_server,
    ...
    for entity in spawned.iter().copied() {
        commands
            .entity(entity)
            .insert((crate::viewport::EditorEntity, Pickable::default()));
    }
