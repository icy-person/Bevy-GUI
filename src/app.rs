...
fn register_default_commands(mut registry: ResMut<EditorCommandRegistry>) {
    for (id, label, shortcut) in [
        ...
        registry.register(EditorCommand {
            id: EditorCommandId(id), label, shortcut,
        });
    }
}
...
fn setup_editor_scene(
    ...
    let cube = commands.spawn((
        ...
        Transform::default(), Pickable::default(), Name::new("Player"),
    )).id();
...
fn keyboard_editor_shortcuts(
    ...
    if ctrl && keys.just_pressed(KeyCode::KeyZ) { history.undo(&mut transforms); project.dirty = true; }
    if ctrl && keys.just_pressed(KeyCode::KeyY) { history.redo(&mut transforms); project.dirty = true; }
    if keys.just_pressed(KeyCode::F6) { project.mode = EditorMode::Play; }
    if keys.just_pressed(KeyCode::F7) { project.mode = EditorMode::Paused; }
    if keys.just_pressed(KeyCode::F8) { project.mode = EditorMode::Edit; }
}

fn editor_mode_tick(_time: Res<Time>, _project: Res<ProjectState>) {}
...
fn record_finished_gizmo_drag(
    ...
    if tracker.active_last_frame && !gizmo_state.active {
        if let Some(entity) = gizmo_state.entity {
            history.push(TransformSnapshot { entity, transform: gizmo_state.start_transform });
            project.dirty = true;
        }
    }
...
fn editor_ui_system(
    ...
    egui::Panel::top("editor_menu").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            for label in ["File", "Edit", "View", "Assets", "Scene", "Entity", "Build"] {
                ui.menu_button(label, |ui| { ui.label("Plugin-provided command surface"); });
            }
            ui.separator();
    ...
    let entities: Vec<(Entity, String)> = names.iter().map(|(entity, name)| (
        entity,
        name.map(|n| n.as_str().to_owned()).unwrap_or_else(|| format!("Entity {:?}", entity)),
    )).collect();
    let selected_transform = selection.entity.and_then(|entity| transforms.get(entity).ok().map(|t| {
    ...
    if create_entity {
        let entity = commands.spawn((Transform::default(), Name::new("Entity"), Pickable::default())).id();
    ...
    if save_requested {
        let items = entities.iter().filter_map(|(entity, name)| transforms.get(*entity).ok().map(|t| (name.clone(), *t)));
        ...
        match save_scene(&path, &document) {
            Ok(()) => { project.dirty = false; state.status = format!("Saved {} entities to {}", document.entities.len(), path.display()); }
...
fn apply_transform_edit(
    ...
    let rotation = Quat::from_euler(EulerRot::XYZ, edit.rotation.x.to_radians(), edit.rotation.y.to_radians(), edit.rotation.z.to_radians());
    let next = Transform { translation: edit.translation, rotation, scale: edit.scale };
    if current != next {
        history.push(TransformSnapshot { entity: edit.entity, transform: current });
...
fn visit_assets(root: &Path, current: &Path, depth: usize, max_depth: usize, max_files: usize, output: &mut Vec<String>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.file_name().is_some_and(|name| name == "target" || name == ".git") { continue; }
