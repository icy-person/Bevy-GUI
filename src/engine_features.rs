...
impl Default for EngineEventMonitor {
    fn default() -> Self { Self::with_capacity(512) }
}

impl EngineEventMonitor {
    pub fn with_capacity(capacity: usize) -> Self {
        Self { capacity: capacity.max(1), sequence: 0, events: VecDeque::new() }
    }
    pub fn push(&mut self, frame: u64, kind: impl Into<String>, payload: impl Into<String>) {
        self.sequence = self.sequence.saturating_add(1);
        self.events.push_back(EngineEventRecord { sequence: self.sequence, frame, kind: kind.into(), payload: payload.into() });
...
impl EngineGraphRegistry {
    ...
    pub fn dependencies_of(&self, name: &str) -> Vec<&EngineSystemInfo> {
        let Some(target) = self.systems.get(name) else { return Vec::new(); };
        target.after.iter().filter_map(|d| self.systems.get(d)).collect()
    }
...
fn register_engine_graph(mut graph: ResMut<EngineGraphRegistry>) {
    ...
    for (name, schedule, reads, writes) in entries {
        graph.register(EngineSystemInfo {
            name: (*name).into(),
...
fn collect_engine_diagnostics(
    ...
    drawables: Query<(), (With<crate::viewport::EditorEntity>, Or<(With<Mesh3d>, With<Camera3d>, With<DirectionalLight>, With<PointLight>, With<SpotLight>)>)>,
    commands: Option<Res<crate::command_executor::CommandExecutionState>>,
    events: Option<Res<EngineEventMonitor>>,
) {
    ...
    if let Some(event_monitor) = events {
        diagnostics.assets_loaded = event_monitor.iter().filter(|event| event.kind == "asset.loaded").count() as u64;
        diagnostics.asset_failures = event_monitor.iter().filter(|event| event.kind == "asset.failed").count() as u64;
    }
}
