...
impl Default for PlacementAxis {
    fn default() -> Self {
        Self::Y
    }
}
...
pub fn update_viewport_cursor(
    ...
    if editor.viewport_mode != ViewportMode::ThreeD || !placement.enabled {
        cursor.clear();
        return;
    }
    ...
    let Ok(ray) = camera.viewport_to_world(transform, screen_position) else {
        cursor.clear();
        return;
    };
    ...
    let grid_size = if placement.grid_size > 0.0 {
        placement.grid_size
    } else {
        settings.settings.viewport.grid_size.max(0.001)
    };
    cursor.world_position = Some(world);
    cursor.grid_position = Some(if placement.snap_to_grid {
        snap_point(world, normal, grid_size)
    } else {
        world
    });
}
...
fn placement_plane(axis: PlacementAxis, offset: f32) -> (Vec3, Vec3) {
    match axis {
        PlacementAxis::X => (Vec3::X, Vec3::X * offset),
        PlacementAxis::Y => (Vec3::Y, Vec3::Y * offset),
        PlacementAxis::Z => (Vec3::Z, Vec3::Z * offset),
    }
}
...
fn snap_point(point: Vec3, normal: Vec3, size: f32) -> Vec3 {
    ...
    if normal.x.abs() > 0.5 {
        result.y = (result.y / size).round() * size;
        result.z = (result.z / size).round() * size;
    } else if normal.y.abs() > 0.5 {
        result.x = (result.x / size).round() * size;
        result.z = (result.z / size).round() * size;
    } else {
        result.x = (result.x / size).round() * size;
        result.y = (result.y / size).round() * size;
...
impl ViewportCursor {
    ...
    pub fn position_or_zero(&self) -> Vec3 {
        self.grid_position.unwrap_or(Vec3::ZERO)
    }
...
mod tests {
    ...
    #[test]
    fn y_plane_snap_preserves_height() {
        let point = Vec3::new(1.49, 0.0, -2.51);
        let snapped = snap_point(point, Vec3::Y, 1.0);
        assert_eq!(snapped, Vec3::new(1.0, 0.0, -3.0));
    }

    #[test]
    fn x_plane_snap_preserves_x() {
        let point = Vec3::new(4.25, 1.49, -2.51);
        let snapped = snap_point(point, Vec3::X, 1.0);
        assert_eq!(snapped, Vec3::new(4.25, 1.0, -3.0));
    }
}
