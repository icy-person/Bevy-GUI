...
use crate::{
    animation::{AnimationClip, AnimationKeyframe, AnimationLibrary, AnimationTrack, KeyValue},
...
struct ToolEntities<'w,'s>{
    all:Query<'w,'s,(Entity,Option<&'static Transform>,Option<&'static Name>,Option<&'static Visibility>,Option<&'static Camera3d>,Option<&'static Mesh3d>,Option<&'static MeshMaterial3d<StandardMaterial>>,Option<&'static avian3d::prelude::RigidBody>,Option<&'static avian3d::prelude::Collider>,Option<&'static AnimationLibrary>,Option<&'static VisualScriptAsset>,Option<&'static crate::EditorPrimitive>,Option<&'static crate::EditorVisual>),With<EditorEntity>>,
    animations:Query<'w,'s,(Entity,&'static AnimationLibrary),With<EditorEntity>>,
    scripts:Query<'w,'s,(Entity,&'static VisualScriptAsset),With<EditorEntity>>,
}

fn draw_engine_tools_ui(mut contexts:EguiContexts,mut ui_state:ResMut<EngineToolsUiState>,editor_state:Res<EditorUiState>,project:Res<ProjectState>,selection:Res<SelectionState>,features:Res<EngineFeatureRegistry>,graph:Res<EngineGraphRegistry>,mut events:ResMut<EngineEventMonitor>,diagnostics:Res<EngineDiagnostics>,registry:Res<ComponentRegistry>,mut animations:Query<(Entity,&mut AnimationLibrary),With<EditorEntity>>,mut shaders:ResMut<ShaderGraphLibrary>,mut commands:Commands,entities:ToolEntities,mut history:ResMut<TransformHistory>,mut project_mut:ResMut<ProjectState>)->Result{
    ...
    egui::Window::new("Bevy-GUI Engine").default_width(480.0).default_height(560.0).resizable(true).show(ctx,|ui|{
        ...
        match ui_state.tab{
            ...
            EngineToolsTab::Animation=>animation_panel(ui,&selection,&mut animations),
            EngineToolsTab::Shaders=>shader_panel(ui,&mut shaders),
            EngineToolsTab::VisualScript=>visual_script_panel(ui,&selection,&mut commands,&entities.scripts,&mut project_mut),
        }
    });
    Ok(())
}
...
fn query_visualizer(ui:&mut egui::Ui,selection:&SelectionState,registry:&ComponentRegistry,entities:&Query<(Entity,Option<&Transform>,Option<&Name>,Option<&Visibility>,Option<&Camera3d>,Option<&Mesh3d>,Option<&MeshMaterial3d<StandardMaterial>>,Option<&avian3d::prelude::RigidBody>,Option<&avian3d::prelude::Collider>,Option<&AnimationLibrary>,Option<&VisualScriptAsset>,Option<&crate::EditorPrimitive>,Option<&crate::EditorVisual>),With<EditorEntity>>){let Some(entity)=selection.primary()else{ui.label("Select an entity to inspect its ECS composition.");return};let Ok((_,transform,name,visibility,camera,mesh,material,body,collider,animation,script,primitive,visual))=entities.get(entity)else{ui.colored_label(egui::Color32::RED,"Selected entity is no longer alive.");return};ui.monospace(format!("Entity {:?}",entity));ui.separator();let actual=[("Transform",transform.is_some()),("Name",name.is_some()),("Visibility",visibility.is_some()),("Camera3d",camera.is_some()),("Mesh3d",mesh.is_some()),("Material",material.is_some()),("RigidBody",body.is_some()),("Collider",collider.is_some()),("Animation",animation.is_some()),("Visual Script",script.is_some()),("Editor Primitive",primitive.is_some()),("Editor Visual",visual.is_some())];ui.strong("Actual components");for(label,present)in actual{ui.label(format!("{} {}",if present{"✓"}else{"○"},label));}ui.separator();ui.strong(format!("Registry: {} components / {} properties",registry.component_count(),registry.property_count()));}
...
fn animation_panel(ui:&mut egui::Ui,selection:&SelectionState,animations:&mut Query<(Entity,&mut AnimationLibrary),With<EditorEntity>>){ui.label(format!("Animation libraries: {}",animations.iter().count()));let Some(selected)=selection.primary()else{ui.label("Select an entity to add or inspect animation.");return};let Ok((entity,mut library))=animations.get_mut(selected)else{if ui.button("Add Demo Animation Library").clicked(){return};ui.label("Selected entity has no AnimationLibrary component yet.");return};ui.horizontal(|ui|{if ui.button("Add Demo Move Clip").clicked(){let clip=AnimationClip{name:"DemoMove".into(),duration:2.0,looping:true,tracks:vec![AnimationTrack{property:"translation".into(),keys:vec![crate::animation::Keyframe{time:0.0,value:KeyValue::Vec3([0.0,0.0,0.0])},crate::animation::Keyframe{time:1.0,value:KeyValue::Vec3([2.0,0.0,0.0])},crate::animation::Keyframe{time:2.0,value:KeyValue::Vec3([0.0,0.0,0.0])}]}]};library.0.insert(clip.name.clone(),clip);ui.label("Added");}});for(name,clip)in &library.0{ui.collapsing(format!("{} · Entity {:?}",name,entity),|ui|{ui.label(format!("{:.2}s · {} tracks · {}",clip.duration,clip.tracks.len(),if clip.looping{"loop"}else{"once"}));})}}
