...
impl Default for ConsolePlugin { fn default() -> Self { Self } }

impl EditorPlugin for SceneEditorPlugin {
    fn name(&self) -> &'static str { "scene-editor" }
    fn build(&self, app: &mut App) {
        app.add_plugins(SceneRuntimePlugin);
        app.world_mut().resource_mut::<EditorPluginRegistry>().register(self.name(), "0.1");
...
impl EditorPlugin for ViewportPlugin {
    ...
    fn build(&self, app: &mut App) {
        app.world_mut().resource_mut::<EditorPluginRegistry>().register(self.name(), "0.1");
        app.world_mut().resource_mut::<PanelRegistry>().register(crate::panel::PanelId("viewport"), "Viewport", viewport_panel);
    }
}
...
impl EditorPlugin for AssetBrowserPlugin {
    ...
    fn build(&self, app: &mut App) {
        app.world_mut().resource_mut::<EditorPluginRegistry>().register(self.name(), "0.1");
        app.world_mut().resource_mut::<PanelRegistry>().register(crate::panel::PanelId("assets"), "Assets", asset_panel);
    }
}
...
impl EditorPlugin for ConsolePlugin {
    ...
    fn build(&self, app: &mut App) {
        app.world_mut().resource_mut::<EditorPluginRegistry>().register(self.name(), "0.1");
        app.world_mut().resource_mut::<PanelRegistry>().register(crate::panel::PanelId("console"), "Console", console_panel);
    }
}
...
fn scene_panel(_world: &mut World, ui: &mut egui::Ui) { ui.label("Scene graph service"); }
fn viewport_panel(_world: &mut World, ui: &mut egui::Ui) { ui.label("Viewport service"); }
fn inspector_panel(_world: &mut World, ui: &mut egui::Ui) { ui.label("Inspector service"); }
fn asset_panel(_world: &mut World, ui: &mut egui::Ui) { ui.label("Asset database service"); }
fn console_panel(_world: &mut World, ui: &mut egui::Ui) { ui.label("Console service"); }
