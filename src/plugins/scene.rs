...
use crate::{
    ...
    scene_tools::{validate_scene, SceneValidationReport},
};
...
fn scene_panel(world: &mut World, ui: &mut egui::Ui) {
    ...
    let Some(project) = project else {
        ui.label("Project state is unavailable.");
        return;
    };
...
fn current_validation(world: &mut World) -> Option<SceneValidationReport> {
    let project = world.get_resource::<ProjectState>()?;
    let main_scene = project.main_scene.as_ref()?;
    let document = crate::scene::load_scene(&project.root.join(main_scene)).ok()?;
    Some(validate_scene(&document))
}
...
fn mode_badge(ui: &mut egui::Ui, mode: EditorMode) {
    let (label, color) = match mode {
        EditorMode::Edit => ("EDIT", egui::Color32::from_rgb(150, 195, 255)),
        EditorMode::Play => ("PLAY", egui::Color32::from_rgb(125, 220, 160)),
        EditorMode::Paused => ("PAUSED", egui::Color32::from_rgb(255, 190, 96)),
    };
