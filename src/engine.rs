...
fn snapshot_editor_scene(entities:&Query<(Entity,&Transform,Option<&Name>,Option<&Visibility>,Option<&Mesh3d>,Option<&MeshMaterial3d<StandardMaterial>>,Option<&EditorParent>,Option<&EditorPrimitive>),With<EditorEntity>>)->crate::scene::SceneDocument{crate::scene::SceneDocument::from_entities_with_visuals(entities.iter().map(|(entity,transform,name,visibility,_mesh,_material,parent,primitive)|{let visual=crate::scene::SceneVisual{primitive:primitive.map(|value|value.0).unwrap_or(ScenePrimitive::None),..Default::default()};(entity,name.map(|value|value.as_str().to_owned()).unwrap_or_else(||"Entity".into()),*transform,parent.and_then(|value|value.0),visibility.map(|value|!matches!(value,Visibility::Hidden)).unwrap_or(true),visual)}))}
fn spawn_runtime_preview(commands:&mut Commands,entities:&Query<(Entity,&Transform,Option<&Name>,Option<&Visibility>,Option<&Mesh3d>,Option<&MeshMaterial3d<StandardMaterial>>,Option<&EditorParent>,Option<&EditorPrimitive>),With<EditorEntity>>){let mut map=HashMap::<Entity,Entity>::new();let mut pending_parents=Vec::new();for(source,transform,name,visibility,mesh,material,parent,_primitive)in entities.iter(){let mut spawned=commands.spawn((RuntimeEntity,*transform,if visibility.map(|value|!matches!(value,Visibility::Hidden)).unwrap_or(true){Visibility::Inherited}else{Visibility::Hidden},Name::new(format!("Runtime: {}",name.map(|value|value.as_str()).unwrap_or("Entity")))));if let Some(mesh)=mesh{spawned.insert(mesh.clone());}if let Some(material)=material{spawned.insert(material.clone());}let runtime_entity=spawned.id();map.insert(source,runtime_entity);pending_parents.push((runtime_entity,parent.and_then(|value|value.0)));}for(runtime_entity,parent)in pending_parents{if let Some(parent_entity)=parent.and_then(|value|map.get(&value).copied()){commands.entity(runtime_entity).insert(ChildOf(parent_entity));}}}
