...
impl Plugin for EngineRuntimePlugin {
    fn build(&self,app:&mut App){app.init_resource::<EngineSettings>().init_resource::<EnginePaths>().init_resource::<EngineRuntimeConfig>().add_systems(Startup,(initialize_runtime_paths,load_configured_scene));}
}
...
impl Plugin for EnginePlugin {fn build(&self,app:&mut App){app.init_resource::<EngineSettings>().init_resource::<EnginePaths>().init_resource::<PlaySession>().add_systems(Startup,initialize_engine_paths).add_systems(Update,sync_editor_runtime_mode);}}
