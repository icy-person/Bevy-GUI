...
impl EngineRuntimeConfig {
    pub fn with_scene(path: impl Into<PathBuf>) -> Self {
        Self {
            main_scene: Some(path.into()),
            ..Default::default()
        }
    }

    pub fn scene_path(&self) -> Option<PathBuf> {
        self.main_scene.as_ref().map(|scene| {
            if scene.is_absolute() {
                scene.clone()
            } else {
                self.project_root.join(scene)
            }
        })
    }
}
...
fn load_configured_scene(
    ...
    let Some(path) = config.scene_path() else {
        return;
    };

    match crate::scene::load_scene(&path) {
        Ok(document) => {
            crate::scene::spawn_scene_with_renderables(
                &mut commands,
                &mut meshes,
                &mut materials,
                &asset_server,
...
) -> Result<Vec<Entity>, crate::scene::SceneIoError> {
    let path = config.scene_path().ok_or_else(|| {
        crate::scene::SceneIoError::Read(std::io::Error::other("no main scene configured"))
    })?;
...
fn snapshot_editor_scene(
    ...
    crate::scene::SceneDocument::from_entities_with_visuals(entities.iter().map(
        |(entity, transform, name, visibility, _mesh, _material, parent, primitive)| {
            let visual = crate::scene::SceneVisual {
                primitive: primitive.map(|value| value.0).unwrap_or(ScenePrimitive::None),
                ..Default::default()
            };
            (
                entity,
    ...
    for (source, transform, name, visibility, mesh, material, parent, _) in entities.iter() {
        let mut spawned = commands.spawn((
            ...
            *transform,
            ...
            Name::new(format!(
                "Runtime: {}",
                name.map(|value| value.as_str()).unwrap_or("Entity")
            )),
        ));
