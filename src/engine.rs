...
impl Plugin for EngineRuntimePlugin{fn build(&self,app:&mut App){app.init_resource::<EngineSettings>().init_resource::<EnginePaths>().init_resource::<EngineRuntimeConfig>().add_plugins(avian3d::prelude::PhysicsPlugins::default()).add_systems(Startup,(initialize_runtime_paths,load_configured_scene));}}
