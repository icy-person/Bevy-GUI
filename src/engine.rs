...
/// Runtime configuration for standalone games generated by Bevy-GUI.
#[derive(Resource, Debug, Clone)]
...
/// Shared runtime plugin for generated standalone Bevy games.
///
/// It owns engine paths/settings but intentionally does not add editor UI.
...
impl Plugin for EngineRuntimePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<EngineSettings>()
            ...
            .add_systems(Startup, initialize_runtime_paths);
    }
}

fn initialize_runtime_paths(
    config: Res<EngineRuntimeConfig>,
    mut paths: ResMut<EnginePaths>,
) {
...
/// Load and spawn the configured authored scene into a standalone Bevy app.
///
/// Returns the spawned entity list and keeps scene IO explicit so callers can
/// handle missing scenes without hiding errors inside the engine.
pub fn load_runtime_scene(
    ...
    let path = config
        .scene_path()
        .ok_or_else(|| crate::scene::SceneIoError::Read(std::io::Error::other("no main scene configured")))?;
    let document = crate::scene::load_scene(&path)?;
    Ok(crate::scene::spawn_scene(commands, &document))
}

/// Editor-side engine plugin. It adds the Play-In-Editor bridge while sharing
/// the same engine settings/paths used by standalone games.
