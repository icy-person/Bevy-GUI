...
impl SceneDocument {
    pub fn from_entities<I>(entities: I) -> Self where I: IntoIterator<Item=(Entity,String,Transform,Option<Entity>)> { build_document(entities.into_iter().map(|(e,n,t,p)|(e,n,t,p,true,SceneNodeKind::Mesh,SceneVisual::default()))) }
    pub fn from_entities_with_visibility<I>(entities: I) -> Self where I: IntoIterator<Item=(Entity,String,Transform,Option<Entity>,bool)> { build_document(entities.into_iter().map(|(e,n,t,p,v)|(e,n,t,p,v,SceneNodeKind::Mesh,SceneVisual::default()))) }
    pub fn from_entities_with_visuals<I>(entities: I) -> Self where I: IntoIterator<Item=(Entity,String,Transform,Option<Entity>,bool,SceneVisual)> { build_document(entities.into_iter().map(|(e,n,t,p,v,s)|(e,n,t,p,v,SceneNodeKind::Mesh,s))) }
    pub fn from_entities_with_kinds<I>(entities: I) -> Self where I: IntoIterator<Item=(Entity,String,Transform,Option<Entity>,bool,SceneNodeKind,SceneVisual)> { build_document(entities) }
}
...
fn spawn_base(commands:&mut Commands, entity:&SceneEntity) -> Entity {
    commands.spawn((Name::new(entity.name.clone()), entity.transform(), if entity.visible { Visibility::Inherited } else { Visibility::Hidden }, SceneNode, crate::EditorParent(None), EditorPrimitive(entity.visual.primitive), EditorVisual(entity.visual.clone()))).id()
}
...
pub fn spawn_scene(commands:&mut Commands, document:&SceneDocument)->Vec<Entity>{
    ...
    for (e,p) in pending { if let Some(parent)=p.and_then(|id|by_id.get(&id).copied()) { commands.entity(e).insert((crate::EditorParent(Some(parent)),ChildOf(parent))); } }
    spawned
}
...
pub fn spawn_scene_with_renderables(commands:&mut Commands,meshes:&mut Assets<Mesh>,materials:&mut Assets<StandardMaterial>,asset_server:&AssetServer,document:&SceneDocument)->Vec<Entity>{
    ...
    for entity in &document.entities {
        ...
        let material=materials.add(StandardMaterial{base_color:Color::srgba(visual.base_color[0],visual.base_color[1],visual.base_color[2],visual.base_color[3]),metallic:visual.metallic.clamp(0.0,1.0),perceptual_roughness:visual.roughness.clamp(0.0,1.0),..default()});
        let e=spawn_base(commands,entity);
        match entity.kind {
            SceneNodeKind::Empty=>{}
            SceneNodeKind::Mesh=>match visual.primitive {
                ScenePrimitive::Cube=>{commands.entity(e).insert((Mesh3d(meshes.add(Cuboid::new(1.0,1.0,1.0))),MeshMaterial3d(material.clone())));}
                ScenePrimitive::Plane=>{commands.entity(e).insert((Mesh3d(meshes.add(Plane3d::default().mesh().size(2.0,2.0))),MeshMaterial3d(material.clone())));}
                ScenePrimitive::Sphere=>{commands.entity(e).insert((Mesh3d(meshes.add(Sphere::new(0.5).mesh().uv(32,18))),MeshMaterial3d(material.clone())));}
                ScenePrimitive::Capsule=>{commands.entity(e).insert((Mesh3d(meshes.add(Capsule3d::new(0.35,0.8))),MeshMaterial3d(material.clone())));}
                ScenePrimitive::None=>{if let Some(asset)=&visual.mesh_asset{commands.entity(e).insert(SceneRoot(asset_server.load(asset.clone())));}}
            }
            SceneNodeKind::Camera3d=>{commands.entity(e).insert(Camera3d::default());}
            SceneNodeKind::DirectionalLight=>{commands.entity(e).insert(DirectionalLight{illuminance:10000.0,..default()});}
            SceneNodeKind::PointLight=>{commands.entity(e).insert(PointLight{intensity:1500000.0,..default()});}
            SceneNodeKind::SpotLight=>{commands.entity(e).insert(SpotLight{intensity:1000000.0,..default()});}
        }
        if visual.collision {
            let collider=match visual.primitive { ScenePrimitive::Sphere=>avian3d::prelude::Collider::sphere(0.5), ScenePrimitive::Capsule=>avian3d::prelude::Collider::capsule(0.35,0.8), _=>avian3d::prelude::Collider::cuboid(0.5,0.5,0.5) };
            match visual.body { SceneBody::Static=>{commands.entity(e).insert((avian3d::prelude::RigidBody::Static,collider));}, SceneBody::Dynamic=>{commands.entity(e).insert((avian3d::prelude::RigidBody::Dynamic,collider));}, SceneBody::Kinematic=>{commands.entity(e).insert((avian3d::prelude::RigidBody::Kinematic,collider));}, SceneBody::None=>{} }
        }
        if let Some(audio)=&visual.audio.asset {
            let settings=if visual.audio.looping { PlaybackSettings::LOOP } else { PlaybackSettings::ONCE }.with_volume(Volume::Linear(visual.audio.volume.clamp(0.0,2.0))).with_spatial(visual.audio.spatial);
            commands.entity(e).insert((AudioPlayer::new(asset_server.load(audio.clone())),settings));
        }
