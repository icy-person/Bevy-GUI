...
impl Default for SceneVisual { fn default()->Self{Self{primitive:ScenePrimitive::None,mesh_asset:None,material_asset:None,base_color:default_color(),metallic:0.0,roughness:0.5,body:SceneBody::None,collision:false}} }
...
#[cfg(test)]mod tests{use super::*;#[test]fn scene_round_trip(){let doc=SceneDocument::from_entities_with_kinds([(Entity::from_bits(1),"Cube".into(),Transform::default(),None,true,SceneNodeKind::Mesh,SceneVisual{primitive:ScenePrimitive::Cube,body:SceneBody::Static,collision:true,..default()})]);let restored:SceneDocument=serde_json::from_str(&serde_json::to_string(&doc).unwrap()).unwrap();assert_eq!(restored.format_version,6);assert_eq!(restored.entities[0].kind,SceneNodeKind::Mesh);assert!(restored.entities[0].visual.collision);}}
