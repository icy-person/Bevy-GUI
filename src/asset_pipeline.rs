...
impl ImportDatabase {
    ...
    pub fn scan_assets(&mut self) -> Vec<PathBuf> {
        let root = self.project_root.join("assets");
            ...
            match self.import_file(&file) {
                Ok(asset) => {
                    if asset.status == ImportStatus::Imported {
                        report.imported += 1;
                    } else if asset.status == ImportStatus::Unsupported {
                        report.unsupported += 1;
                    }
                    self.assets
                        .insert(normalize_key(&file), asset);
                }
                Err(error) => {
                    report.failed += 1;
    ...
    pub fn import_file(&self, source: &Path) -> Result<ImportedAsset, ImportError> {
        let relative = source
            .strip_prefix(self.project_root.join("assets"))
            .unwrap_or(source)
            .to_path_buf();
        let metadata = fs::metadata(source).map_err(|source_error| ImportError::Read {
        ...
        let kind = source
            ...
            .map(ImportKind::from_extension)
            .unwrap_or(ImportKind::Unknown);
        if kind == ImportKind::Unknown && self.settings.fail_on_unknown {
        ...
        let mut asset = ImportedAsset {
            ...
            generated: Vec::new(),
            kind,
            status: if kind == ImportKind::Unknown {
        ...
        let destination = self.imported_root.join(&relative);
        if self.settings.copy_sources {
            if let Some(parent) = destination.parent() {
                fs::create_dir_all(parent).map_err(ImportError::CreateDirectory)?;
            }
            fs::copy(source, &destination).map_err(|source_error| ImportError::Copy {
                path: source.to_path_buf(),
                source: source_error,
            })?;
    ...
    pub fn save(&self) -> Result<(), ImportError> {
        let path = self.project_root.join(".bevy-gui").join("imports.json");
...
pub enum ImportError {
    ...
    Unsupported(PathBuf),
    #[error("failed to create import directory: {0}")]
    CreateDirectory(#[source] io::Error),
    #[error("failed to copy asset {path}: {source}")]
    Copy { path: PathBuf, source: io::Error },
    #[error("failed to serialize metadata: {0}")]
    Serialize(#[source] serde_json::Error),
    #[error("failed to write metadata: {0}")]
    Write(#[source] io::Error),
    #[error("failed to read import database: {0}")]
    DatabaseRead(#[source] io::Error),
    #[error("failed to parse import database: {0}")]
...
fn visit_assets(
    ...
    if depth > max_depth || output.len() >= max_files || !current.exists() {
        return;
    }
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            visit_assets(&path, output, depth + 1, max_depth, max_files);
