...
fn interpolate(a: KeyValue, b: KeyValue, t: f32) -> Option<KeyValue> {
    match (a, b) {
        (KeyValue::Float(a), KeyValue::Float(b)) => Some(KeyValue::Float(a + (b - a) * t)),
        (KeyValue::Vec3(a), KeyValue::Vec3(b)) => Some(KeyValue::Vec3([
            a[0] + (b[0] - a[0]) * t,
            a[1] + (b[1] - a[1]) * t,
            a[2] + (b[2] - a[2]) * t,
        ])),
        (KeyValue::Bool(a), KeyValue::Bool(b)) => Some(KeyValue::Bool(if t < 1.0 { a } else { b })),
        _ => None,
    }
}
...
impl AnimatorState {
    ...
    pub fn advance(&mut self, delta: f32, duration: f32, looping: bool) {
        ...
        if self.time >= duration && duration > 0.0 {
            if looping { self.time %= duration; } else { self.time = duration; self.playing = false; }
        }
    }
}
...
impl Plugin for AnimationRuntimePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, advance_animators);
    }
}
...
mod tests {
    use super::*;
    #[test]
    fn numeric_keys_interpolate() {
        let clip = AnimationClip { name: "move".into(), duration: 2.0, looping: false, tracks: vec![AnimationTrack { property: "translation".into(), keys: vec![Keyframe { time: 0.0, value: KeyValue::Vec3([0.0, 0.0, 0.0]) }, Keyframe { time: 2.0, value: KeyValue::Vec3([4.0, 2.0, 0.0]) }] }] };
        assert_eq!(clip.sample("translation", 1.0), Some(KeyValue::Vec3([2.0, 1.0, 0.0])));
    }
    #[test]
    fn animator_loops() {
        let mut animator = AnimatorState::default(); animator.play("walk"); animator.advance(1.2, 1.0, true);
        assert!((animator.time - 0.2).abs() < 0.001); assert!(animator.playing);
    }
    #[test]
    fn clip_clamps_after_last_key() {
        let clip = AnimationClip { name: "move".into(), duration: 2.0, looping: false, tracks: vec![AnimationTrack { property: "x".into(), keys: vec![Keyframe { time: 0.0, value: KeyValue::Float(0.0) }, Keyframe { time: 1.0, value: KeyValue::Float(5.0) }] }] };
        assert_eq!(clip.sample("x", 9.0), Some(KeyValue::Float(5.0)));
    }
}
