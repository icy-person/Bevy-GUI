...
pub fn execute_editor_commands(
    mut bus:ResMut<EditorCommandBus>,mut project:ResMut<ProjectState>,mut assets:ResMut<AssetDatabase>,mut imports:ResMut<ImportDatabase>,mut state:ResMut<CommandExecutionState>,mut selection:ResMut<SelectionState>,cursor:Res<ViewportCursor>,mut history_events:EventWriter<HistoryCommandEvent>,mut commands:Commands,mut meshes:ResMut<Assets<Mesh>>,mut materials:ResMut<Assets<StandardMaterial>>,asset_server:Res<AssetServer>,
    transforms:Query<(Entity,&Transform,Option<&Name>,Option<&EditorParent>,Option<&Visibility>,Option<&EditorPrimitive>,Option<&EditorVisual>),With<EditorEntity>>,names:Query<(Entity,Option<&Name>,Option<&EditorParent>,Option<&Visibility>),With<EditorEntity>>,
){
    for id in bus.drain(){
        state.executed=state.executed.saturating_add(1);state.last=Some(id);state.last_error=None;state.last_message=None;
        match id.0{
            "project.save"|"scene.save"=>{if save_current_scene(&project,&transforms,&mut state){project.dirty=false;}}
...
fn save_current_scene(project:&ProjectState,transforms:&Query<(Entity,&Transform,Option<&Name>,Option<&EditorParent>,Option<&Visibility>,Option<&EditorPrimitive>,Option<&EditorVisual>),With<EditorEntity>>,state:&mut CommandExecutionState)->bool{
    ...
    match crate::scene::save_scene(&project.root.join(scene),&document){Ok(())=>{state.last_message=Some(format!("Saved {} scene entities",document.entities.len()));true},Err(error)=>{state.last_error=Some(error.to_string());false}}
}
...
fn open_current_scene(commands:&mut Commands,selection:&mut SelectionState,project:&ProjectState,asset_server:&AssetServer,meshes:&mut Assets<Mesh>,materials:&mut Assets<StandardMaterial>,current:&Query<(Entity,&Transform,Option<&Name>,Option<&EditorParent>,Option<&Visibility>,Option<&EditorPrimitive>,Option<&EditorVisual>),With<EditorEntity>>,state:&mut CommandExecutionState){
    ...
    match load_scene(&project.root.join(scene)){Ok(document)=>{for(entity,_,_,_,_,_,_)in current.iter(){commands.entity(entity).despawn();}selection.entities.clear();selection.focused=None;let spawned=crate::scene::spawn_scene_with_renderables(commands,meshes,materials,asset_server,&document);for entity in spawned.iter().copied(){commands.entity(entity).insert(EditorEntity);}state.last_message=Some(format!("Opened scene with {} entities",spawned.len()));},Err(error)=>state.last_error=Some(error.to_string())}
}

fn spawn_editor_entity(commands:&mut Commands,selection:&mut SelectionState,project:&mut ProjectState,state:&mut CommandExecutionState,name:&str,primitive:Option<ScenePrimitive>,transform:Transform){let primitive=primitive.unwrap_or(ScenePrimitive::None);let entity=commands.spawn((transform,Name::new(name),Visibility::Inherited,EditorEntity,EditorParent(None),Pickable::default(),EditorPrimitive(primitive),EditorVisual(SceneVisual{primitive,..default()}))).id();selection.select(entity);project.dirty=true;state.last_message=Some(format!("{name} created"));}
