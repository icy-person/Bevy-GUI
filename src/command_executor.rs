...
pub fn execute_editor_commands(
    ...
    transforms:Query<(Entity,&Transform,Option<&Name>,Option<&EditorParent>,Option<&Visibility>,Option<&EditorPrimitive>),With<EditorEntity>>,names:Query<(Entity,Option<&Name>,Option<&EditorParent>,Option<&Visibility>),With<EditorEntity>>,
){
    for id in bus.drain(){
        state.executed=state.executed.saturating_add(1);state.last=Some(id);state.last_error=None;state.last_message=None;
        match id.0{
            "project.save"=>{
                if let Some(scene)=&project.main_scene{let document=crate::scene::SceneDocument::from_entities_with_visuals(transforms.iter().map(|(entity,transform,name,parent,visibility,primitive)|(entity,name.map(|v|v.as_str().to_owned()).unwrap_or_else(||"Entity".into()),*transform,parent.and_then(|v|v.0),visibility.map(|v|!matches!(v,Visibility::Hidden)).unwrap_or(true),SceneVisual{primitive:primitive.map(|v|v.0).unwrap_or(ScenePrimitive::None),..default()})));if let Err(error)=crate::scene::save_scene(&project.root.join(scene),&document){state.last_error=Some(error.to_string());continue;}}
            ...
            "project.play"=>project.mode=EditorMode::Play,"project.pause"=>project.mode=EditorMode::Paused,"project.stop"=>project.mode=EditorMode::Edit,
            ...
            "scene.duplicate"=>{if let Some(source)=selection.primary(){if let Ok((_,transform,name,_,_,primitive))=transforms.get(source){let copy_name=name.map(|n|format!("{} Copy",n.as_str())).unwrap_or_else(||"Duplicate".into());let entity=commands.spawn((*transform,Name::new(copy_name),Visibility::Inherited,EditorEntity,EditorParent(None),Pickable::default(),EditorPrimitive(primitive.map(|p|p.0).unwrap_or(ScenePrimitive::None)))).id();selection.select(entity);project.dirty=true;state.last_message=Some("Entity duplicated".into())}else{state.last_error=Some("Selected entity is no longer available".into())}}else{state.last_message=Some("Select an entity first".into())}},
